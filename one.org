...
pattern to it. Whatever I intuitively comes to mind I post.
...
:END:
** Google Pixel 7
Terminess is installed to remotely connect to my VPS. I access emacs and tmux
exclusively through this.

Termux is setup
- Daily Driver Phone 
- All applications disabled apart from messaging and a web browser.
- Great Google integration. No USB-C Alt Mode support because greed.
- Termux and Todoist are manly used.

** Thinkpad X220
Favorite laptop of recent years. Running Debian. Corbooted.

** Portable Laptop
CrowView portable monitor with keyboard and battery with the RPI 5 adapter installed.
<insert image here>

** Remote server
Remote into a VPS to run emacs and tmux.

** Freedom Mode
Thinkpad with Tail's linux.
